...
Показанные на рисунке параметры находятся в узле RawGoods. "C" - цена в долларах(USD) - вывести в рублях(курс выбрать произвольно), "G" - id группы, "T" - id товара, "P" - сколько единиц товара осталось (параметр, который указан в скобках в названии).
...
Дополнительная информация: Дизайну, показанному в примерах, следовать не обязательно. Надо реализовать адаптив (мобильная и таблет версии) для списка товаров. Добавить анимацию появления и удаления товара из списка товаров. Прокомментировать основные действия. Интересные решения приветствуются.